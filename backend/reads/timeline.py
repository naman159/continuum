...
def list_timeline(db: Any, novel_id: UUID | str, up_to_chapter: int | None) -> list[dict[str, Any]]:
    ...
    char_rows = db.fetchall("SELECT id, name FROM characters WHERE novel_id = %s", (novel_id,), dict_rows=True)
    char_name = {r["id"]: r["name"] for r in char_rows}
    loc_rows = db.fetchall("SELECT id, name FROM locations WHERE novel_id = %s", (novel_id,), dict_rows=True)
    loc_name = {r["id"]: r["name"] for r in loc_rows}
    obj_rows = db.fetchall("SELECT id, name FROM objects WHERE novel_id = %s", (novel_id,), dict_rows=True)
    obj_name = {r["id"]: r["name"] for r in obj_rows}
    faction_rows = db.fetchall("SELECT id, name FROM factions WHERE novel_id = %s", (novel_id,), dict_rows=True)
